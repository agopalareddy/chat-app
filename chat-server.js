  ...
  cleanupMessages: () => {
    ...
    ['rooms', 'private'].forEach((type) => {
      for (const [id, messages] of state.messages[type].entries()) {
        state.messages[type].set(
          id,
          messages.filter((m) => m.timestamp > dayAgo)
        );
      }
    });
  },
...
io.sockets.on('connection', (socket) => {
  ...
  const updateRoomUsers = (room) => {
    ...
    const bannedUsers = state.bannedUsers.has(room) ? Array.from(state.bannedUsers.get(room)) : [];
  ...
  socket.on('set_nickname', (nickname) => {
    ...
    if (!isValidNickname(trimmed)) {
      return socket.emit('error_message', {
        message:
          'Invalid nickname. Use 1-30 alphanumeric characters, spaces, underscores, or hyphens.',
      });
    }
    ...
    if (
      oldNickname &&
      oldNickname !== trimmed &&
      state.nicksToSockets.get(oldNickname) === socket.id
    ) {
  ...
  socket.on('join_room', (data) => {
    ...
    if (kickRemaining && !isRoomOwner) {
      return socket.emit('error_message', {
        message: `You are kicked from this room for ${kickRemaining} more minutes`,
      });
    }
    ...
    if (
      !isRoomOwner &&
      state.roomPasswords.has(room) &&
      state.roomPasswords.get(room) !== '' &&
      state.roomPasswords.get(room) !== password
    ) {
    ...
    if (socket.currentRoom) {
      ...
      if (state.usersByRoom[socket.currentRoom]) {
        state.usersByRoom[socket.currentRoom] = state.usersByRoom[socket.currentRoom].filter(
          (id) => id !== socket.id
        );
  ...
  socket.on('kick_user', (data) => {
    ...
    if (
      roomOwner !== kickerNickname ||
      !duration ||
      duration < KICK_DURATION_MIN ||
      duration > KICK_DURATION_MAX
    ) {
  ...
  socket.on('private_message', (data) => {
    ...
    [toSocketId, socket.id].forEach((id) => io.to(id).emit('private_message', messageData));
