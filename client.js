...
function updateTypingIndicator() {
  ...
  if (typists.length === 1) el.textContent = typists[0] + ' is typing...';
    ...
    el.textContent = typists[0] + ' and ' + typists[1] + ' are typing...';
...
function renderMessageContent(message, formattedTime, from) {
  ...
  return (
    timeHtml + ' <span class="msg-author" style="color:' + color + '">' + from + '</span>: ' + text
  );
}
...
function createMessageElement(containerId, data) {
  ...
  messageDiv.className =
    'message ' + (isOwn ? 'message-own' : 'message-other') + (data.replyTo ? ' reply' : '');
  messageDiv.id = 'message-' + data.messageId;

  let html = '';
  if (data.replyTo && data.replyTo.messageId) {
    html +=
      '<div class="reply-indicator" onclick="scrollToMessage(\'' +
      (data.replyTo.messageId || '') +
      '\')">\u21AA Replying to ' +
      (data.replyTo.from || '') +
      ': ' +
      (data.replyTo.preview || '').substring(0, 30) +
      '</div>';
  }
  ...
  html +=
    '<button title="Reply" aria-label="Reply" onclick="replyToMessage(\'' +
    (data.messageId || '') +
    "','" +
    (data.from || '') +
    '\',this.closest(\'.message\'))"><span class="material-icons">reply</span></button>';
  if (data.from === state.nickname) {
    ...
    html +=
      '<button title="Delete" aria-label="Delete" onclick="deleteMessage(\'' +
      (data.messageId || '') +
      "','" +
...
function switchChat(chatId) {
  ...
  document.querySelectorAll('.private-chat').forEach(function (chat) {
...
function kickUser(userId) {
  ...
  form.innerHTML =
    '<label>Duration (min): <input type="number" class="kick-duration" min="1" max="1440" value="5"></label><button class="kick-confirm">Kick</button><button class="kick-cancel">Cancel</button>';
  ...
  form.querySelector('.kick-cancel').addEventListener('click', function () {
    form.remove();
  });
}
...
function unbanUser(userId) {
  socketio.emit('unban_user', { userId: userId, room: state.currentRoom });
}
function unkickUser(userId) {
  socketio.emit('unkick_user', { userId: userId, room: state.currentRoom });
}
...
function openPrivateChat(otherUser) {
  ...
  if (!state.activePrivateChats.has(otherUser)) {
    ...
    newTab.innerHTML =
      otherUser +
      ' <span class="tab-close" onclick="closePrivateChat(\'' +
      otherUser +
      '\',event)"><span class="material-icons">close</span></span><span class="unread-badge hidden"></span>';
    newTab.addEventListener('click', function () {
      switchChat('private-' + otherUser);
    });
    newTab.addEventListener('keypress', function (e) {
      if (e.key === 'Enter') switchChat('private-' + otherUser);
    });
...
function replyToMessage(messageId, from, messageElement) {
  const messageText =
    messageElement.getAttribute('data-message-text') || messageElement.textContent || '';
...
function showReplyIndicator(from) {
  ...
  el.innerHTML =
    ...
    from +
    ': ' +
    (replyState.replyingTo.preview || '') +
    '</span><button class="cancel-reply" onclick="cancelReply()" aria-label="Cancel reply"><span class="material-icons">close</span></button>';
...
function logout() {
  showConfirm('Are you sure you want to log out?').then(function (yes) {
    ...
    document.getElementById('chat-tabs').innerHTML =
      '<div class="chat-tab active" data-chat="room" id="roomChatTab" role="tab" tabindex="0" aria-selected="true">Room Chat</div>';
...
function createRoomElement(room, isCurrentRoom) {
  ...
  div.addEventListener('click', function () {
    joinRoom(room.name, room.hasPassword, room.owner);
  });
  div.addEventListener('keypress', function (e) {
    if (e.key === 'Enter') joinRoom(room.name, room.hasPassword, room.owner);
  });
  return div;
}
...
function createUserElement(user, roomOwner) {
  ...
  div.addEventListener('click', function () {
    if (user !== state.nickname) openPrivateChat(user);
  });
  div.addEventListener('keypress', function (e) {
    if (e.key === 'Enter' && user !== state.nickname) openPrivateChat(user);
  });
  return div;
}
...
socketio.on('join_room_success', function (data) {
  ...
  if (data.messageHistory && data.messageHistory.length) {
    ...
    empty.innerHTML =
      '\u{1F44B} <strong>Welcome to ' + data.room + '!</strong><br>Be the first to say hello.';
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('roomChatTab').addEventListener('click', function () {
    switchChat('room');
  });
